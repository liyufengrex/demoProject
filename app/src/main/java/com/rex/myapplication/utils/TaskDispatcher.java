...
/**
 * @Description: TODO
 * @Author: 李宇峰 on 2019-05-22 9:18 AM.
 * @NO.: 146714
 * @Phone: 13756017116
...
public class TaskDispatcher {
    ...
    static {
        ...
        //标准的后台程序
        HandlerThread handlerThread = new HandlerThread("TaskDispatcher-Computation", android.os.Process.THREAD_PRIORITY_BACKGROUND);
        //有效线程的最低优先级
        HandlerThread ioThread = new HandlerThread("TaskDispatcher-IO", android.os.Process.THREAD_PRIORITY_LOWEST);
        handlerThread.start();
        ioThread.start();
        sThreadHandler = new Handler(handlerThread.getLooper());
        sIOHandler = new Handler(ioThread.getLooper());
        sMainHandler = new Handler(Looper.getMainLooper());
        sExecutor = new ThreadPoolExecutor(coreThreadCount, coreThreadCount + 1, 60L, TimeUnit.SECONDS, workQueue, new TaskDispatcher.TaskDispatcherThreadFactory());
    }
